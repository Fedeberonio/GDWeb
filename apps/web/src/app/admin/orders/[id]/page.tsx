...
export default function OrderDetailPage() {
  ...
  const executeFinalize = async (data: ConfirmationData) => {
    ...
    try {
      ...
      const msg = `Hola ${data.customerName}! Tu pedido #${orderId.slice(0, 6)} está confirmado.`;
  ...
  const filteredProducts = products
    .filter(
      (p) =>
        p.name.es.toLowerCase().includes(productSearchQuery.toLowerCase()) ||
        ...
        <main className="max-w-[1400px] mx-auto p-6 grid grid-cols-12 gap-6">
            ...
            <section className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 relative overflow-hidden flex flex-col sm:flex-row gap-6 items-center sm:items-start">
              ...
              <div className="flex-1 z-10 text-center sm:text-left">
                ...
                <div className="flex flex-wrap items-center justify-center sm:justify-start gap-4 mt-2 text-slate-600">
                  ...
                  {editedOrder.guestEmail && (
                    <div className="flex items-center gap-1.5 bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-100 text-sm">
                      <Mail className="w-4 h-4 text-emerald-600" /> {editedOrder.guestEmail}
                    </div>
                  )}
                </div>
                {(customer?.likes || customer?.dislikes) && (
                  <div className="mt-4 flex flex-wrap justify-center sm:justify-start gap-2">
                    {customer.likes?.split(",").map((tag, i) => (
                      <span
                        key={i}
                    ...
                    {customer.dislikes?.split(",").map((tag, i) => (
                      <span
                        key={i}
                              ...
                              >
                                <div>
                                  ...
                                  <p className="text-xs text-slate-400">
                                    {(typeof product.unit === "string" ? product.unit : product.unit?.es) || "Unidad"}
                                  </p>
                                </div>
                    ...
                    {editedOrder.items.map((item, idx) => (
                      <tr key={idx} className="group hover:bg-gray-50/80 transition-colors">
                        <td className="pl-6 py-3">
                          <div className="font-semibold text-slate-700">{item.name.es}</div>
                          {item.metadata?.unit && <div className="text-xs text-slate-400">{item.metadata.unit}</div>}
                        </td>
          ...
          <div className="col-span-12 lg:col-span-4 space-y-6">
                      ...
                      setEditedOrder((prev) =>
                        prev
                          ? {
                              ...
                              delivery: {
                                ...prev.delivery,
                                window: { ...prev.delivery.window, slot: e.target.value },
                              },
                            }
                          : prev,
                      )
            ...
            <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 space-y-3">
              ...
              <button
                onClick={() => (hasUnsavedChanges ? toast.error("Guarda los cambios primero") : setShowConfirmModal(true))}
                disabled={isFinalizing}
                className="w-full py-4 bg-emerald-700 hover:bg-emerald-800 active:scale-[0.98] text-white rounded-xl font-bold shadow-lg shadow-emerald-200 transition-all flex items-center justify-center gap-2 text-lg disabled:opacity-60"
              >
            ...
            <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 space-y-2 text-sm text-slate-600">
              ...
              <div>Origen: {editedOrder.metadata?.source || "—"}</div>
            </div>
          </div>
        </main>
